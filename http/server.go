...
func (c *conn) readRequest() (w *response, err error) {
	...
	if c.server.XHeaders {
		req.RemoteAddr = req.Header.Get("X-Forwarded-For")
		// Nginx X-Real-IP: http://wiki.nginx.org/HttpRealipModule
		/*
			if req.RemoteAddr == "" {
	...
	if req.RemoteAddr == "" {
		req.RemoteAddr = "unix" // Unix Socket clients without XHeaders
	}
	req.TLS = c.tlsState
...
// Status returns the HTTP status of the response, if there's one.
// Must be called after WriterHeader, otherwise returns zero.
...
func (w *response) finishRequest() {
	...
	// Call the logger
	if w.conn.server.Logger != nil {
		w.conn.server.Logger(w, w.req)
	}
}
...
func (c *conn) serve() {
	...
	for {
		...
		if err != nil {
			...
			// Logger can't be called because there's no Request
			break
		}
...
type Server struct {
	Addr           string        // TCP or Unix Socket address to listen on, ":http" if empty
	Handler        Handler       // handler to invoke, http.DefaultServeMux if nil
	Logger         HandlerFunc   // handler function, called at the end of every request
	ReadTimeout    time.Duration // maximum duration before timing out read of the request
	WriteTimeout   time.Duration // maximum duration before timing out write of the response
	MaxHeaderBytes int           // maximum size of request headers, DefaultMaxHeaderBytes if 0
	TLSConfig      *tls.Config   // optional TLS config, used by ListenAndServeTLS
	XHeaders       bool          // use X-Forwarded-For or X-Real-IP headers when available
}

// ListenAndServe listens on the TCP network address or Unix Socket path
// srv.Addr and then calls Serve to handle requests on incoming connections.
// If srv.Addr is blank, ":http" is used.
// To listen on Unix Socket, srv.Addr must start with unix: and point to a
// file path. e.g.: unix:/tmp/myserver.
